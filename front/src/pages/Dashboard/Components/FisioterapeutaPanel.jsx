// components/FisioterapeutaPanel.jsx
import React from 'react';
import { FiChevronDown, FiChevronUp } from 'react-icons/fi';
import { AnimatePresence, motion } from 'framer-motion';

export default function FisioterapeutaPanel(props) {
  const {
  ...
  return (
    <section className={`${themeWithSpec.card} ${theme.bg}`}>
      <header className="flex flex-col md:flex-row items-start md:items-center justify-between gap-3 mb-6">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold tracking-tight">Painel — Fisioterapeuta</h1>
          <p className={`text-sm mt-1 ${theme.muted}`}>Gerencie pacientes e suas sessões.</p>
        </div>
          ...
          <div className={`${themeWithSpec.panel} p-3 rounded-2xl`}>
            <div className="flex gap-4">
              ...
              <div className="flex-1">
                ...
                <div className="text-xs text-gray-400">Especialidade: Fisioterapeuta</div>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className={`${themeWithSpec.panel} rounded-2xl p-3`}>
            <div className="flex items-center justify-between mb-2">
              ...
              <div className={`${theme.muted} text-sm`}>Gerencie pacientes e sessões</div>
            </div>
      ...
      <footer className={`mt-6 text-sm ${theme.muted} text-center`}>Painel Fisioterapeuta</footer>
    </section>
  );
}
